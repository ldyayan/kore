...
extern struct char_data *get_exchar_vis(struct char_data *ch, char *name);
...
int find_first_step(sh_int src, sh_int target)
{
  ...
  for (curr_dir = 0; curr_dir < NUM_OF_DIRS - 1; curr_dir++)
...
ACMD(do_track)
{
  ...
  if (!IS_NPC(ch) && !IS_THIEF(ch) && !IS_WARRIOR(ch) && !IS_BARD(ch) &&
      !IS_UNDEAD(ch) && !IS_DRAGON(ch) && !IS_DEATHKNIGHT(ch) &&
      !IS_DRUID(ch)) {
    send_to_char("You fail to find a trail.\r\n", ch);
    return;
  }
...
/* HACKED to make the mob being tracked notrack if you fail */
/* HACKED so that its no biggy if you fail your track (by disabling code) */
/*
	...
	dir = number(0, NUM_OF_DIRS - 1);
      ...
      if (IS_NPC(vict))
        SET_BIT(AFF_FLAGS(vict), AFF_NOTRACK);
    }
*/
/* end of hack */
