...
/* HACKED, since we don't HAVE a creation port! */
/*  if ((GET_LEVEL(ch) < LVL_SUPR) && (port != creation_port)) {
    sprintf(buf, "Please use the creation port, port %d, to build on.\r\n",
        creation_port);
    send_to_char(buf, ch);
    return;
  } */
  ...
  /* the olc zone is saved one larger than it should be because of zone 0 */
  /* dont let even IMPS edit zones they're not assigned to */
...
/*
	...
	send_to_char("You need to set your olc to this zone.\r\n", ch);
	free(d->olc);
    }
*/
...
{ struct olc_save_info *entry;
  ...
  {
    *buf2 = '\0';
    if (entry->zone == GET_OLC_ZONE(ch) - 1)
      sprintf(buf2, "(yours)");
    sprintf(buf, " - %s for zone %d. %s\r\n", 
