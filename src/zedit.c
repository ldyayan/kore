...
extern char *zone_bits[];		/*. constants.c .*/
...
{ FILE *fp;
  ...
  sprintf(buf, "%s/%.3i.zon", ZON_PREFIX, vzone_num);
  ...
  /*. Create Rooms file .*/
  sprintf(buf, "%s/%.3d.wld", WLD_PREFIX, vzone_num);
  if(!(fp = fopen(buf, "w"))) 
  ...
  /*. Create Mobiles file .*/
  sprintf(buf, "%s/%.3i.mob", MOB_PREFIX, vzone_num);
  if(!(fp = fopen(buf, "w")))
  ...
  /*. Create Objects file .*/
  sprintf(buf, "%s/%.3i.obj", OBJ_PREFIX, vzone_num);
  if(!(fp = fopen(buf, "w"))) 
  ...
  /*. Create Shops file .*/
  /* This was buggy, I fixed it, then decided I didn't want it in anyway */
/*  sprintf(buf, "%s/%.3i.shp", SHP_PREFIX, vzone_num);
  if(!(fp = fopen(buf, "w")))
...
*/
  sprintf(buf, "rm %s", old_name);
  system(buf);
  sprintf(buf, "mv %s %s", new_name, old_name);
  system(buf);
...
{ int subcmd, arg1 = -1, arg2 = -1, arg3 = -1;
  ...
  sprintf(fname, "%s/%.3i.zon", ZON_PREFIX, 
  ...
  sprintf(buf, 
	...
	zone_table[OLC_ZNUM(d)].name ? zone_table[OLC_ZNUM(d)].name : "undefined",
 	zone_table[OLC_ZNUM(d)].top,
        ...
        zone_table[OLC_ZNUM(d)].zone_flags
  );
  fprintf(zfile, buf);
  ...
  for(subcmd = 0; ZCMD.command != 'S'; subcmd++) {
    *arg4 = '\0';
	...
	arg3 = world[ZCMD.arg3].number;
        sprintf(arg4, "%s", mob_proto[ZCMD.arg1].player.short_descr);
	break;
      case 'O':
	arg1 = obj_index[ZCMD.arg1].virtual;
	arg2 = ZCMD.arg2;
	arg3 = world[ZCMD.arg3].number;
        sprintf(arg4, "%s", obj_proto[ZCMD.arg1].short_description);
	break;
      case 'G':
 	arg1 = obj_index[ZCMD.arg1].virtual;
	arg2 = ZCMD.arg2;
	...
	arg2 = ZCMD.arg2;
	arg3 = ZCMD.arg3;
        sprintf(arg4, "        %s %s", equipment_types[ZCMD.arg3],
            obj_proto[ZCMD.arg1].short_description);
	break;
      case 'P':
	arg1 = obj_index[ZCMD.arg1].virtual;
	...
	arg2 = ZCMD.arg2;
	arg3 = ZCMD.arg3;
        sprintf(arg4, "%s door %s", world[ZCMD.arg1].name, dirs[ZCMD.arg2]);
	break;
      case 'R':
	arg1 = world[ZCMD.arg1].number;
	arg2 = obj_index[ZCMD.arg2].virtual;
	arg3 = -1;
        sprintf(arg4, "remove %s",
...
{ int counter, columns = 0;
  ...
  { sprintf(buf, "%s%2d%s) %-20.20s ",
            grn, counter + 1, nrm, zone_bits[counter]);
    if(!(++columns % 2))
      strcat(buf, "\r\n");
    send_to_char(buf, d->character);
  }
  ...
  case ZEDIT_FLAGS:
    ...
    if ((number < 0) || (number > NUM_ZONE_FLAGS)) {
      ...
      if (number == 0) {
        ...
        /* toggle bits */
        if (IS_SET(OLC_ZONE(d)->zone_flags, 1 << (number - 1)))
          REMOVE_BIT(OLC_ZONE(d)->zone_flags, 1 << (number - 1));
        else
          SET_BIT(OLC_ZONE(d)->zone_flags, 1 << (number - 1));
        zedit_disp_flag_menu(d);
      }
    }
    zedit_disp_menu(d);
