...
/* boards, now in clan.c cause thats where it's getting changed */
...
/* HACKED to save class and clan into the boards */
  NEW_MSG_INDEX(board_type).heading[len - 1] = '\0';
  NEW_MSG_INDEX(board_type).level = GET_LEVEL(ch);
  NEW_MSG_INDEX(board_type).class = GET_CLASS(ch);
  NEW_MSG_INDEX(board_type).clan = GET_CLAN(ch);
/* end of hack */
...
/* HACKED to change boards if they're looking at clan boards */
/* old viewing code */
/*
...
/* HACKED to save board class and clan */
  for (; ind < num_of_msgs[board_type] - 1; ind++) {
    ...
    MSG_CLASS(board_type, ind) = MSG_CLASS(board_type, ind + 1);
    MSG_CLAN(board_type, ind) = MSG_CLAN(board_type, ind + 1);
  }
/* end of hack */
  num_of_msgs[board_type]--;
